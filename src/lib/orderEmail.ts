...
/**
 * New-order alert to the sales inbox.
 *
 * Structural input types rather than `Prisma.OrderGetPayload<...>`: the caller
 * already has the order in hand from its own `include`, and pinning this to one
 * exact payload shape would break every time that include changed. Decimal
 * columns arrive as Prisma.Decimal, which serialises via String(), so numeric
 * fields accept `number | string` and are normalised here.
 */

/** Anything with a Decimal-ish runtime value. */
 ...
 * Every interpolated value below is customer-controlled — a product name or a
 * landmark containing `<` would otherwise corrupt the markup, and in a webmail
 * client that renders HTML it is a script-injection vector.
 */
...
/** IST, because the business reads these and the servers run in UTC. */
...
function renderItems(items: OrderEmailItem[]): string {
  return items
    .map((item) => {
      const sku = item.sku ? `<br /><span style="color:#9ca3af;font-size:12px;">SKU: ${escapeHtml(item.sku)}</span>` : "";
...
/**
 * Table-based layout with inline styles throughout. Not a stylistic choice:
 * Gmail strips <style> blocks and Outlook's Word renderer ignores flex/grid, so
 * inline attributes on nested tables are what actually renders consistently.
 */
...
/** Plain-text alternative, so spam filters don't mark this HTML-only. */
function buildText(order: OrderEmailPayload): string {
  const lines = order.items.map(
    (item) =>
      `  - ${item.productName} x${item.quantity} — ${formatCurrency(Number(item.totalAmount))}`
  );
...
/**
 * Sends the new-order alert. Resolves `false` instead of throwing on every
 * failure path, so a caller can never break a placed order by awaiting it — the
 * order is already committed by the time this runs, and a dead SMTP host must
 * not turn a successful checkout into an error the customer sees.
 */
export async function sendOrderNotification(
  ...
  if (!transporter || !from) {
    console.warn(
      `[orderEmail] SMTP not configured — skipping alert for ${order.invoiceNumber}`
    );
    return false;
  }
  ...
  if (!recipient) {
    console.warn(
      `[orderEmail] ORDER_NOTIFICATION_EMAIL not set — skipping alert for ${order.invoiceNumber}`
    );
    return false;
  }
  ...
  try {
    await sendMail({
      ...
      subject: `New order ${order.invoiceNumber} — ${formatCurrency(
    ...
    // Logged, never rethrown. The order is already saved; losing the alert is
    // recoverable from the admin panel, losing the order is not.
    console.error(
      `[orderEmail] failed to send alert for ${order.invoiceNumber}:`,
      error
    );
